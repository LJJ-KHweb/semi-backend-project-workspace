...
<img width="1536" height="1024" alt="ChatGPT Image 2026년 7월 16일 오전 11_18_26" src="https://github.com/user-attachments/assets/53736991-92a2-45c7-9e93-1b530062ff1b" />

<br/>

### ERD 설계 설명

* `SEMI_USER`를 중심으로 게시판, 공지사항, 문의, 차량, 상품, 충전소 기능을 연결했습니다.
* **Sequence**를 사용하여 각 테이블의 기본 키를 자동 생성하도록 설계했습니다.
* **STATUS 컬럼**을 활용해 데이터를 실제로 삭제하지 않는 **Soft Delete 방식**을 적용했습니다.
* 첨부파일, 재고, 토큰, 주행 기록은 별도 테이블로 분리하여 데이터 중복을 줄였습니다.
* 테이블 간 관계는 **Foreign Key**로 연결하여 데이터 무결성을 유지했습니다.
* 충전소와 충전기, 게시글과 첨부파일처럼 하나의 데이터에 여러 데이터가 연결되는 경우 **1:N 관계**로 설계했습니다.

> 📖 각 테이블을 **왜 이렇게 설계했는지**, 컬럼·시퀀스·관계별 의도를 정리한 상세 문서 → **[ERD 설계 의도 자세히 보기](./docs/ERD.md)**
...
* **잘못된 토큰인데 요청이 계속 진행** — `JwtFilter`의 예외 분기에 `return`이 빠져, 401 응답 후에도 필터 체인이 이어집니다. → catch 블록에 `return` 추가.
* **파일 URL이 localhost 고정** — 저장 파일 경로가 `http://localhost/...`로 하드코딩되어 배포 환경에 대응하지 못합니다. → base-url을 설정 파일에서 주입.
* **업로드 파일 검증 부재** — 확장자·용량·MIME 검사가 없고 파일명 충돌 여지가 있습니다. → UUID 파일명 + 확장자 화이트리스트 + 용량 제한.
* **재고 차감의 경합 조건** — 재고 확인과 차감 사이에 다른 요청이 끼어들 수 있습니다. → `WHERE AMOUNT > 0` 조건을 걸고 갱신 행이 없으면 예외 처리.

>  위 항목들은 외부 지적이 아니라 팀이 스스로 회고하며 찾아낸 것으로, 다음 프로젝트에서는 이를 초기 체크리스트로 두고 시작할 계획입니다.
